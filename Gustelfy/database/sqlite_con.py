...
class SqliteCon(DatabaseConnection):
    ...
    def connect_database(self):
        ...
        self.connection = sqlite3.connect(settings.get_dbpath())
    ...
    def ensure_validity(self):
        '''Ensures that database structure is fitting to the requirements. Attempts to fix it in case it isn't.'''
        default_structure = helper.json_to_dict(os.path.join(settings.get_datafolder(),"sqlite3_conf.json"))

        for table in default_structure:
            if not self._table_exists(table):
                logger.error(f"Table {table} is missing. Creating replacement...")
    ...
    def table_exists(self, table: str) -> bool:
        ...
        result = self.cursor.execute("SELECT name FROM sqlite_schema WHERE type ='table' AND name = ?;", (tablename,)).fetchall()

        if len(result) == 0:
            return False
        if len(result) == 1:
            return True
        logger.critical("Table appears to exist multiple times. Database may be inconsistent.")
